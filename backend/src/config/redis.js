 ...
 * Can be disabled via REDIS_DISABLED=true or DISABLE_REDIS=true environment variable.
 */

// Check if Redis is explicitly disabled
const isRedisDisabled = () => {
  const disabled = process.env.REDIS_DISABLED === 'true' || process.env.DISABLE_REDIS === 'true'
  if (disabled) {
    logger.info('Redis explicitly disabled via environment variable')
  }
  return disabled
}
...
if (!isRedisDisabled() && REDIS_URL) {
  ...
  logger.info('Redis disabled - running without Redis (development mode)')
} else {

...
DO $$ BEGIN
    CREATE TYPE ride_status AS ENUM (
        ...
        'in_progress','completed','cancelled','no_driver_available'
    );
...
CREATE TABLE IF NOT EXISTS rides (
    ...
    surge_multiplier      DECIMAL(3,2)  DEFAULT 1.0
                          CHECK (surge_multiplier >= 1.0),
    preferred_vehicle_type VARCHAR(20)
                          CHECK (preferred_vehicle_type IN
                            ('sedan','suv','van','luxury','motorcycle','electric')),
    passenger_count       INTEGER       DEFAULT 1
                          CHECK (passenger_count > 0 AND passenger_count <= 8),
